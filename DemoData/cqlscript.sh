...
cqlsh `hostname -i` -e "COPY  killr_video.videos_by_actor (actor_name,character_name,video_id,release_year,title) FROM 'videos_by_actor.csv'"
cqlsh `hostname -i` -e "COPY  killr_video.videos (video_id,avg_rating,description,genres,mpaa_rating,release_date,release_year,title,user_id) FROM 'videos.csv'"
